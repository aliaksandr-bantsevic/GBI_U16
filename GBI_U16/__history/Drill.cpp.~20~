...
int TDrill::AddMeas(TStringGrid* t, AnsiString n)
{
	...
	m->mark.printf("Маркировка Измерение_%d", meas_list_idx+1);
...
int TDrill::Excel(void)
{
	...
	system(cmd);
			...
			{
				Application->MessageBoxA(L"Не удалось создать таблицу!",L"ОШИБКА",0);
				return -1;
			}
	...
	for (int i = 0; i < meas_list_idx; i++) {
			...
			{
					Application->MessageBox(L"Не удалось экспортировать измерение!",L"ОШИБКА",0);
				return -2;
			}
	}
