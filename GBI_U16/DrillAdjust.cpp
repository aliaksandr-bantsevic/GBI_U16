	 ...
	 if (mode == 0) {
		 ...
		 *records_cnt = this->ComboBox_meas_cnt->Text.ToInt()+1;
			...
			if (MAX_RECORDS_MEAS  < *records_cnt) {
				...
				utils_ShowMessage("Превышен лимит числа уровней!");
				return;
...
void __fastcall TForm_DrillAdjust::Timer_startTimer(TObject *Sender)
{
	...
	if (mode == 0) {
		...
		Caption = "Добавить скважину";
		this->Edit_name->Text = "Скважина";
...
void __fastcall TForm_DrillAdjust::FormCreate(TObject *Sender)
{
	...
	this->ComboBox_orient->Items->Add("Горизонтальная");
	this->ComboBox_orient->Items->Add("Вертикальная");
