...
void __fastcall TForm_SensorAdjust::FormCreate(TObject *Sender)
{
	...
	this->ComboBox_protocol->Items->Add("ИН-Д3");
	this->ComboBox_protocol->Items->Add("АН-Д3");
...
void __fastcall TForm_SensorAdjust::Button_port_openClick(TObject *Sender)
{
	...
	else
	{
		...
		else
		{
			this->Button_port_open->Caption = "Открыть порт";
			this->Edit_port_prop->Color = clSilver;
			Application->MessageBoxA(L"Не удалось открыть порт", L"Ошибка",0);
			this->Button_start->Enabled = false;
        }
	}
}
		...
		if (scan_addr>=254) {
			...
			this->Button_start->Caption = "Старт";
			 ...
			 if (total_sensors>0)
			 {
				 ...
				 {
					if (Application->MessageBoxA(L"Адрес первого обнаруженного датчика отличается от настроек системы. Сохраниьт в настройках новое значение адреса?",L"Внимание!",1)==IDOK) {
